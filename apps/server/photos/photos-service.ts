...
export class PhotosService {
  ...
  private async populatePhotoAlbums(
    ...
    const albumPromises = photosOnAlbums
      ...
      .map((relation) => db.albums.find((album) => album.id === relation.albumId))

    const albums = await Promise.all(albumPromises)

    return {
      ...photo,
      albums: albums.filter((album): album is Album => album !== undefined) || []
    }
  }
  ...
  async getPhotoById(
    ...
    (Photo & { nextPhotoId: string | null; previousPhotoId: string | null }) | null
  > {
    const db = await this.dbService.readDatabase()
  ...
  async uploadImage(
    ...
    return await this.populatePhotoAlbums(db.photos[photoIndex], db.photosOnAlbums)
  }
  ...
  async deletePhoto(id: string): Promise<boolean> {
    ...
    db.photosOnAlbums = db.photosOnAlbums.filter((relation) => relation.photoId !== id)
  ...
  async managePhotoAlbums(
    ...
    db.photosOnAlbums = db.photosOnAlbums.filter(
      (relation) =>
        relation.photoId !== photoId || !albumsToRemove.includes(relation.albumId)
    )
